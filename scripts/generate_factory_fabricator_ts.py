#!/usr/bin/env python3
"""Generează factoryFabricatorVideos.ts și factoryFabricatorPageHotspots.ts din manifest."""
...
VIDEOS_TS = ROOT / "src/data/factoryFabricatorVideos.ts"
HOTSPOTS_TS = ROOT / "src/data/factoryFabricatorPageHotspots.ts"
...
def main() -> None:
    ...
    videos_lines = [
        "/** Videoclipuri Fabricator fabrică stocate local (public/docs/equipment/factory-fabricator/videos/). */",
    ]
    if video_ids:
        videos_lines.append(f'const VIDEO = (id: string) => `{videos_dir}/${{id}}.mp4`;')
        videos_lines.append("")
    videos_lines.append("export const FACTORY_FABRICATOR_LOCAL_VIDEOS: Record<string, string> = {")
    ...
    hotspot_lines = [
        ...
        "",
        "export const FACTORY_FABRICATOR_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
    ]
    if data["pageHotspots"]:
        hotspot_lines = [
            "import { FACTORY_FABRICATOR_LOCAL_VIDEOS } from '@/data/factoryFabricatorVideos';",
            "import type { EquipmentManualPageVideoHotspot } from '@/data/equipmentOperations';",
            "",
            "export const FACTORY_FABRICATOR_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
        ]
    for page_str, spots in sorted(data["pageHotspots"].items(), key=lambda x: int(x[0])):
        ...
        for spot in spots:
            ...
            hotspot_lines.append(
                f"    {{ videoUrl: FACTORY_FABRICATOR_LOCAL_VIDEOS['{vid}'], "
