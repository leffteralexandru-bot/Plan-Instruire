#!/usr/bin/env python3
"""Generează prodimCtVideos.ts și prodimCtPageHotspots.ts din manifest."""
...
VIDEOS_TS = ROOT / "src/data/prodimCtVideos.ts"
HOTSPOTS_TS = ROOT / "src/data/prodimCtPageHotspots.ts"
...
def main() -> None:
    ...
    videos_lines = [
        "/** Videoclipuri Prodim CT stocate local (public/docs/equipment/prodim-ct/videos/). */",
        f'const VIDEO = (id: string) => `{videos_dir}/${{id}}.mp4`;',
        "",
        "export const PRODIM_CT_LOCAL_VIDEOS: Record<string, string> = {",
    ]
    ...
    hotspot_lines = [
        "import { PRODIM_CT_LOCAL_VIDEOS } from '@/data/prodimCtVideos';",
        "import type { EquipmentManualPageVideoHotspot } from '@/data/equipmentOperations';",
        "",
        "export const PRODIM_CT_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
    ]
    for page_str, spots in sorted(data["pageHotspots"].items(), key=lambda x: int(x[0])):
        ...
        for spot in spots:
            ...
            hotspot_lines.append(
                f"    {{ videoUrl: PRODIM_CT_LOCAL_VIDEOS['{vid}'], "
