#!/usr/bin/env python3
"""Generează prolinerNewRemoteVideos.ts și prolinerNewRemotePageHotspots.ts din manifest."""
...
VIDEOS_TS = ROOT / "src/data/prolinerNewRemoteVideos.ts"
HOTSPOTS_TS = ROOT / "src/data/prolinerNewRemotePageHotspots.ts"
...
def main() -> None:
    ...
    videos_lines = [
        "/** Videoclipuri CT 3.1 TELECOMANDĂ NOUĂ (public/docs/equipment/proliner-new-remote/videos/). */",
    ]
    if video_ids:
        videos_lines.append(f'const VIDEO = (id: string) => `{videos_dir}/${{id}}.mp4`;')
        videos_lines.append("")
    videos_lines.append("export const PROLINER_NEW_REMOTE_LOCAL_VIDEOS: Record<string, string> = {")
    ...
    hotspot_lines = [
        ...
        "",
        "export const PROLINER_NEW_REMOTE_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
    ]
    if data["pageHotspots"]:
        hotspot_lines = [
            "import { PROLINER_NEW_REMOTE_LOCAL_VIDEOS } from '@/data/prolinerNewRemoteVideos';",
            "import type { EquipmentManualPageVideoHotspot } from '@/data/equipmentOperations';",
            "",
            "export const PROLINER_NEW_REMOTE_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
        ]
    for page_str, spots in sorted(data["pageHotspots"].items(), key=lambda x: int(x[0])):
        ...
        for spot in spots:
            ...
            hotspot_lines.append(
                f"    {{ videoUrl: PROLINER_NEW_REMOTE_LOCAL_VIDEOS['{vid}'], "
