#!/usr/bin/env python3
"""Generează prolinerRemoteVideos.ts și prolinerRemotePageHotspots.ts din manifest."""
...
VIDEOS_TS = ROOT / "src/data/prolinerRemoteVideos.ts"
HOTSPOTS_TS = ROOT / "src/data/prolinerRemotePageHotspots.ts"
...
def main() -> None:
    ...
    videos_lines = [
        "/** Videoclipuri PROLINER TELECOMANDĂ stocate local (public/docs/equipment/proliner-remote/videos/). */",
    ]
    if video_ids:
        videos_lines.append(f'const VIDEO = (id: string) => `{videos_dir}/${{id}}.mp4`;')
        videos_lines.append("")
    videos_lines.append("export const PROLINER_REMOTE_LOCAL_VIDEOS: Record<string, string> = {")
    ...
    hotspot_lines = [
        ...
        "",
        "export const PROLINER_REMOTE_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
    ]
    if data["pageHotspots"]:
        hotspot_lines = [
            "import { PROLINER_REMOTE_LOCAL_VIDEOS } from '@/data/prolinerRemoteVideos';",
            "import type { EquipmentManualPageVideoHotspot } from '@/data/equipmentOperations';",
            "",
            "export const PROLINER_REMOTE_PAGE_HOTSPOTS: Record<number, EquipmentManualPageVideoHotspot[]> = {",
        ]
    for page_str, spots in sorted(data["pageHotspots"].items(), key=lambda x: int(x[0])):
        ...
        for spot in spots:
            ...
            hotspot_lines.append(
                f"    {{ videoUrl: PROLINER_REMOTE_LOCAL_VIDEOS['{vid}'], "
